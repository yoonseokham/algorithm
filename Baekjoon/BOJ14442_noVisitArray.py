...
def BFS(maze:list,k)->int:
    ...
    visit=[ [ [False]*len(maze[0]) for i in range(len(maze))] for _ in range(k+1) ]
    visit[0][0][0]=True
    answer=1000001
    ...
    while q:
        ...
        for i,j in indexInfo:
            ...
            if 0<=newI<len(maze) and 0<=newJ<len(maze[0]):
                if maze[newI][newJ]==0  and not visit[curSmashed][newI][newJ]:
                    q.append((newI,newJ,curSmashed,curMoved+1))
                    visit[curSmashed][newI][newJ]=True
                if curSmashed<k and maze[newI][newJ]==1 and not visit[curSmashed+1][newI][newJ]:
                    q.append((newI,newJ,curSmashed+1,curMoved+1))
                    visit[curSmashed+1][newI][newJ]=True
